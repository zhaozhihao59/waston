package com.tocersoft.subscribe.action.front;
...
@ParentPackage("front")
@Namespace("/front/subscribe")
@Controller
public class SubscribeFrontAction extends BaseAction implements ModelDriven<SubscribeModel>{
 ...
 */
	 ...
	 * 
	 */
	@Action(value = "cancle_subscribe", results = { @Result(name = "success", location = "/WEB-INF/pages/front/subscribe/cancel_subscribe_success.jsp") })
	public String cancle_subscribe(){
		try {
			Subscribe item = subscribeService.getSubscribeItemByemail(model.getEmail(),model.getChannelId());
			model.setItem(item);
