...
@Namespace("/member/order/sell")
@Controller
public class OrderSellMemberAction extends BaseAction implements ModelDriven<OrderSellModel>{

	private static final long serialVersionUID = 1L;

	private Log logger = LogFactory.getLog(OrderSellMemberAction.class);
	...
	@Action(value = "buy", results = { 
			@Result(name = "success", location = "/WEB-INF/pages/front/order/order_confirm.jsp"),
			@Result(name = "cart_list",type="redirectAction",params={"namespace","/cart","actionName","index"})
			}
	)
	public String buy(){
		// 查询用户 收货地址
		...
		if(StringUtils.isNotBlank(cartItemStrs)){
			...
			for(OrderCartItem cartItem : orderCartItemList){
				...
				if(cartItemStrs.indexOf(cartItem.getId()) >= 0){
					...
					//删除加入订单的购物车明细
					orderCartItemService.delByIds(cartItemStrs.split(","));
				}
			}
...
//						}
	...
	@Action(value = "confirm", results = { 
			@Result(name = "success",type="redirectAction",params={"namespace","/member/order/pay","actionName","detail","item.id","${item.id}"}),
			@Result(name = "order_list",type="redirectAction",params={"namespace","/member/account","actionName","my_order"})
			}
	)
