...
(function(k) {
	var E, F, G, H, I, J, r = {}, K = {}, s = {}, L = {
		...
		callback : {
			...
			onRemove : null
				...
				a.bind(c.ASYNC_ERROR, function(a, c, h, l, g, e) {
							j
									.apply(b.callback.onAsyncError, [a, c, h,
													l, g, e])
						})
			}], u = [function(b) {
		var a = e.event;
		...
		if (j.eqs(b.type, "mousedown"))
			...
			if (j.eqs(a.tagName, "span")
				...
				d = (k(a).parent("li").get(0) || k(a).parentsUntil("li")
						.parent().get(0)).id, h = "switchNode";
			else {
				if (q = j.getMDom(c, a, [{
									tagName : "a",
									attrName : "treeNode" + e.id.A
								}]))
					d = (k(q).parent("li").get(0) || k(q).parentsUntil("li")
							.parent().get(0)).id, h = "clickNode"
			}
		else if (j.eqs(b.type, "dblclick")
				&& (l = "dblclick", q = j.getMDom(c, a, [{
									tagName : "a",
									attrName : "treeNode" + e.id.A
								}])))
			d = (k(q).parent("li").get(0) || k(q).parentsUntil("li").parent()
					.get(0)).id, h = "switchNode";
		if (l.length > 0 && d.length == 0 && (q = j.getMDom(c, a, [{
							tagName : "a",
							attrName : "treeNode" + e.id.A
						}])))
			d = (k(q).parent("li").get(0) || k(q).parentsUntil("li").parent()
		...
		addCreatedNode : function(b, a) {
			(b.callback.onNodeCreated || b.view.addDiyDom)
					&& g.getRoot(b).createdNodes.push(a)
		},
		...
		getNextNode : function(b, a) {
			...
			for (var c = b.data.key.children, d = a.parentTId ? a
					.getParentNode() : g.getRoot(b), f = 0, h = d[c].length - 1; f <= h; f++)
				if (d[c][f] === a)
					return f == h ? null : d[c][f + 1];
			return null
		},
		...
		getNodesByParam : function(b, a, c, d) {
			if (!a || !c)
				return [];
			for (var f = b.data.key.children, h = [], l = 0, e = a.length; l < e; l++)
				a[l][c] == d && h.push(a[l]), h = h.concat(g.getNodesByParam(b,
						a[l][f], c, d));
			return h
		},
		getNodesByParamFuzzy : function(b, a, c, d) {
			if (!a || !c)
				return [];
			for (var f = b.data.key.children, h = [], l = 0, e = a.length; l < e; l++)
				typeof a[l][c] == "string" && a[l][c].indexOf(d) > -1
						&& h.push(a[l]), h = h.concat(g.getNodesByParamFuzzy(b,
						a[l][f], c, d));
			return h
		},
		...
		getPreNode : function(b, a) {
			if (!a)
				return null;
			for (var c = b.data.key.children, d = a.parentTId ? a
					.getParentNode() : g.getRoot(b), f = 0, h = d[c].length; f < h; f++)
				if (d[c][f] === a)
		...
		removeSelectedNode : function(b, a) {
			for (var c = g.getRoot(b), d = 0, f = c.curSelectedList.length; d < f; d++)
				if (a === c.curSelectedList[d]
						|| !g.getNodeCache(b, c.curSelectedList[d].tId))
		...
		transformToArrayFormat : function(b, a) {
			...
			if (j.isArray(a))
				for (var f = 0, h = a.length; f < h; f++)
					d.push(a[f]), a[f][c]
							&& (d = d.concat(g.transformToArrayFormat(b,
									a[f][c])));
			else
				d.push(a), a[c]
						&& (d = d.concat(g.transformToArrayFormat(b, a[c])));
			return d
		},
		...
		proxy : function(b) {
			...
			if (!j.uCanDo(a, b))
				return !0;
			for (var c = m.doProxy(b), d = !0, f = !1, h = 0, e = c.length; h < e; h++) {
				var i = c[h];
				i.nodeEventCallback
						&& (f = !0, d = i.nodeEventCallback.apply(i,
								[b, i.node])
								&& d);
				i.treeEventCallback
						&& (f = !0, d = i.treeEventCallback.apply(i,
								[b, i.node])
								&& d)
			}
			try {
	...
	F = function(b, a) {
		var c = g.getSetting(b.data.treeId), d = c.view.autoCancelSelected
				&& b.ctrlKey && g.isSelectedNode(c, a)
				? 0
	...
	var j = {
		...
		clone : function(b) {
			...
			for (c in b)
				b.hasOwnProperty(c)
						&& (a[c] = typeof b[c] === "object" ? arguments
								.callee(b[c]) : b[c]);
			return a
		},
		eqs : function(b, a) {
		...
		addNodes : function(b, a, c, d) {
			if (!b.data.keep.leaf || !a || a.isParent)
				if (j.isArray(c) || (c = [c]), b.data.simpleData.enable
						&& (c = g.transformTozTreeFormat(b, c)), a) {
					var f = k("#" + a.tId + e.id.SWITCH), h = k("#" + a.tId
							+ e.id.ICON), l = k("#" + a.tId + e.id.UL);
					if (!a.open)
						i.replaceSwitchClass(a, f, e.folder.CLOSE), i
								.replaceIcoClass(a, h, e.folder.CLOSE), a.open = !1, l
								.css({
											display : "none"
										});
					...
					g.addNodesData(b, g.getRoot(b), c), i.createNodes(b, 0, c,
							null)
		},
		appendNodes : function(b, a, c, d, f, h) {
			if (!c)
				return [];
			for (var e = [], j = b.data.key.children, k = 0, q = c.length; k < q; k++) {
				var o = c[k];
				if (f) {
					var m = (d ? d : g.getRoot(b))[j].length == c.length
							&& k == 0;
				...
				o[j]
						&& o[j].length > 0
						&& (m = i
								.appendNodes(b, a + 1, o[j], o, f, h && o.open));
				h
						&& (i.makeDOMNodeMainBefore(e, b, o), i
								.makeDOMNodeLine(e, b, o), g
								.getBeforeA(b, o, e), i.makeDOMNodeNameBefore(
								e, b, o), g.getInnerBeforeA(b, o, e), i
								.makeDOMNodeIcon(e, b, o), g.getInnerAfterA(b,
								o, e), i.makeDOMNodeNameAfter(e, b, o), g
								.getAfterA(b, o, e), o.isParent && o.open
								&& i.makeUlHtml(b, o, e, m.join("")), i
								.makeDOMNodeMainAfter(e, b, o), g
								.addCreatedNode(b, o))
			}
			return e
		},
		appendParentULDom : function(b, a) {
			var c = [], d = k("#" + a.tId), f = k("#" + a.tId + e.id.UL), h = i
					.appendNodes(b, a.level + 1, a[b.data.key.children], a, !1,
							!0);
			i.makeUlHtml(b, a, c, h.join(""));
			!d.get(0)
					&& a.parentTId
					&& (i.appendParentULDom(b, a.getParentNode()), d = k("#"
							+ a.tId));
			f.get(0) && f.remove();
			d.append(c.join(""))
		},
		asyncNode : function(b, a, c, d) {
			...
			if (a && !a.isParent)
				...
				return j.apply(d), !1;
			if (a)
				a.isAjaxing = !0, k("#" + a.tId + e.id.ICON).attr({
							style : "",
			...
			for (f = 0, h = b.async.autoParam.length; a && f < h; f++) {
				...
				l
						? (n = typeof a[m] == "string" ? '"' : "", p += '"' + o
								+ ('":' + n + a[m]).replace(/'/g, "\\'") + n
								+ ",")
						: p += o + ("=" + a[m]).replace(/&/g, "%26") + "&"
			}
			if (j.isArray(b.async.otherParam))
				for (f = 0, h = b.async.otherParam.length; f < h; f += 2)
					l
							? (n = typeof b.async.otherParam[f + 1] == "string"
									? '"'
									: "", p += '"'
									+ b.async.otherParam[f]
									+ ('":' + n + b.async.otherParam[f + 1])
											.replace(/'/g, "\\'") + n + ",")
							: p += b.async.otherParam[f]
									+ ("=" + b.async.otherParam[f + 1])
											.replace(/&/g, "%26") + "&";
			else
				for (var r in b.async.otherParam)
					l ? (n = typeof b.async.otherParam[r] == "string"
							? '"'
							: "", p += '"'
							+ r
							+ ('":' + n + b.async.otherParam[r]).replace(/'/g,
									"\\'") + n + ",") : p += r
							+ ("=" + b.async.otherParam[r])
									.replace(/&/g, "%26") + "&";
			...
			k.ajax({
				...
				success : function(f) {
					if (s == g.getRoot(b)._ver) {
						var h = [];
						try {
							h = !f || f.length == 0 ? [] : typeof f == "string"
									? eval("(" + f + ")")
									: f
						} catch (l) {
							h = f
						...
						h && h !== "" ? (h = j.apply(b.async.dataFilter, [
										b.treeId, a, h], h), i.addNodes(b, a, h
										? j.clone(h)
										: [], !!c)) : i.addNodes(b, a, [], !!c);
		...
		createNodes : function(b, a, c, d) {
			if (c && c.length != 0) {
				var f = g.getRoot(b), h = b.data.key.children, h = !d || d.open
						|| !!k("#" + d[h][0].tId).get(0);
				f.createdNodes = [];
				a = i.appendNodes(b, a, c, d, !0, h);
				d ? (d = k("#" + d.tId + e.id.UL), d.get(0)
		...
		destroy : function(b) {
			b
					&& (g.initCache(b), g.initRoot(b), m.unbindTree(b), m
		...
		expandCollapseNode : function(b, a, c, d, f) {
			...
			if (a) {
				...
				if (a.open == c)
					...
					var c = k("#" + a.tId + e.id.UL), h = k("#" + a.tId
							+ e.id.SWITCH), n = k("#" + a.tId + e.id.ICON);
					a.isParent
		...
		expandCollapseSonNode : function(b, a, c, d, f) {
			var h = g.getRoot(b), e = b.data.key.children, h = a ? a[e] : h[e], e = a
					? !1
					: d, j = g.getRoot(b).expandTriggerFlag;
			g.getRoot(b).expandTriggerFlag = !1;
			if (h)
				for (var k = 0, m = h.length; k < m; k++)
					h[k] && i.expandCollapseSonNode(b, h[k], c, e);
			g.getRoot(b).expandTriggerFlag = j;
			i.expandCollapseNode(b, a, c, d, f)
		},
		makeDOMNodeIcon : function(b, a, c) {
			var d = g.getNodeName(a, c), d = a.view.nameIsHTML ? d : d.replace(
		...
		makeDOMNodeMainBefore : function(b, a, c) {
			b.push("<li id='", c.tId, "' class='level", c.level,
		...
		makeDOMNodeNameBefore : function(b, a, c) {
			var d = g.getNodeTitle(a, c), f = i.makeNodeUrl(a, c), h = i
					.makeNodeFontCss(a, c), l = [], k;
			for (k in h)
				l.push(k, ":", h[k], ";");
			b.push("<a id='", c.tId, e.id.A, "' class='level", c.level,
					"' treeNode", e.id.A, ' onclick="', c.click || "", '" ',
			...
			if (a) {
				...
				if (d) {
					for (var f = 0, h = d.length; f < h; f++)
						g.removeNodeCache(b, d[f]);
					g.removeSelectedNode(b);
					delete a[c];
					b.data.keep.parent
							? k("#" + a.tId + e.id.UL).empty()
							: (a.isParent = !1, a.open = !1, c = k("#" + a.tId
									+ e.id.SWITCH), d = k("#" + a.tId
									+ e.id.ICON), i.replaceSwitchClass(a, c,
									e.folder.DOCU), i.replaceIcoClass(a, d,
									e.folder.DOCU), k("#" + a.tId + e.id.UL)
									.remove())
				}
			}
		},
		...
		removeNode : function(b, a) {
			var c = g.getRoot(b), d = b.data.key.children, f = a.parentTId ? a
					.getParentNode() : c;
			a.isFirstNode = !1;
			a.isLastNode = !1;
			...
			if (g.getNodeCache(b, a.tId)) {
				...
				i.setLastNode(b, f);
				var j, h = f[d].length;
				if (!b.data.keep.parent && h == 0)
					f.isParent = !1, f.open = !1, h = k("#" + f.tId + e.id.UL), l = k("#"
							+ f.tId + e.id.SWITCH), j = k("#" + f.tId
							+ e.id.ICON), i.replaceSwitchClass(f, l,
							e.folder.DOCU), i.replaceIcoClass(f, j,
							e.folder.DOCU), h.css("display", "none");
				else if (b.view.showLine && h > 0) {
					var n = f[d][h - 1], h = k("#" + n.tId + e.id.UL), l = k("#"
							+ n.tId + e.id.SWITCH);
					j = k("#" + n.tId + e.id.ICON);
					f == c ? f[d].length == 1 ? i.replaceSwitchClass(n, l,
							e.line.ROOT) : (c = k("#" + f[d][0].tId
							+ e.id.SWITCH), i.replaceSwitchClass(f[d][0], c,
							e.line.ROOTS), i.replaceSwitchClass(n, l,
							e.line.BOTTOM)) : i.replaceSwitchClass(n, l,
							e.line.BOTTOM);
		...
		setNodeLineIcos : function(b, a) {
			if (a) {
				var c = k("#" + a.tId + e.id.SWITCH), d = k("#" + a.tId
						+ e.id.UL), f = k("#" + a.tId + e.id.ICON), h = i
						.makeUlLineClass(b, a);
		...
		setNodeName : function(b, a) {
			...
			d.empty();
			b.view.nameIsHTML ? d.html(g.getNodeName(b, a)) : d.text(g
					.getNodeName(b, a));
			j.apply(b.view.showTitle, [b.treeId, a], b.view.showTitle)
					&& k("#" + a.tId + e.id.A).attr("title", !c ? "" : c)
		},
	...
	k.fn.zTree = {
		...
		init : function(b, a, c) {
			...
			d.treeObj.empty();
			r[d.treeId] = d;
			if (k.browser.msie && parseInt(k.browser.version) < 7)
				d.view.expandSpeed = "";
			g.initRoot(d);
			b = g.getRoot(d);
			...
			c = {
				...
				getNodeByParam : function(a, b, c) {
					return !a ? null : g.getNodeByParam(this.setting, c
				...
				getNodesByParam : function(a, b, c) {
					return !a ? null : g.getNodesByParam(this.setting, c
									? c[this.setting.data.key.children]
									: g.getNodes(this.setting), a, b)
				},
				getNodesByParamFuzzy : function(a, b, c) {
					return !a ? null : g.getNodesByParamFuzzy(this.setting, c
									? c[this.setting.data.key.children]
									: g.getNodes(this.setting), a, b)
				},
				...
				getNodeIndex : function(a) {
					...
					for (var b = d.data.key.children, c = a.parentTId ? a
							.getParentNode() : g.getRoot(this.setting), e = 0, i = c[b].length; e < i; e++)
						if (c[b][e] == a)
							return e;
					return -1
				},
				...
				reAsyncChildNodes : function(a, b, c) {
					if (this.setting.async.enable) {
						...
						if (b == "refresh") {
							for (var b = this.setting.data.key.children, m = 0, q = a[b].length; m < q; m++)
								g.removeNodeCache(d, a[b][m]);
							g.removeSelectedNode(d);
							a[b] = [];
							j ? this.setting.treeObj.empty() : k("#" + a.tId
									+ e.id.UL).empty()
						}
...
(function(m) {
	...
	m.extend(!0, m.fn.zTree._z, {
		...
		view : {
			checkNodeRelation : function(c, a) {
				...
				if (c.check.chkStyle == b.STYLE) {
					...
					if (a[h])
						if (c.check.radioType == b.TYPE_ALL) {
							for (d = i.length - 1; d >= 0; d--)
								b = i[d], b[h] = !1, i.splice(d, 1), e
										.setChkClass(c, m("#" + b.tId
														+ j.id.CHECK), b), b.parentTId != a.parentTId
										&& e.repairParentChkClassWithSelf(c, b);
							i.push(a)
						} else {
							i = a.parentTId ? a.getParentNode() : f.getRoot(c);
							for (d = 0, g = i[l].length; d < g; d++)
								b = i[l][d], b[h]
										&& b != a
										&& (b[h] = !1, e.setChkClass(c, m("#"
														+ b.tId + j.id.CHECK),
												b))
						}
					...
					a[h]
							&& (!a[l] || a[l].length == 0 || c.check.chkboxType.Y
									.indexOf("s") > -1)
							&& e.setSonNodeCheckBox(c, a, !0), !a[h]
							&& (!a[l] || a[l].length == 0 || c.check.chkboxType.N
									.indexOf("s") > -1)
							&& e.setSonNodeCheckBox(c, a, !1), a[h]
							&& c.check.chkboxType.Y.indexOf("p") > -1
							&& e.setParentNodeCheckBox(c, a, !0), !a[h]
							&& c.check.chkboxType.N.indexOf("p") > -1
							&& e.setParentNodeCheckBox(c, a, !1)
			},
			makeChkClass : function(c, a) {
				var b = c.data.key.checked, d = j.checkbox, g = j.radio, e = "", e = a.chkDisabled === !0
						? d.DISABLED
						: a.halfCheck ? d.PART : c.check.chkStyle == g.STYLE
						...
						+ (a[b] ? d.TRUE : d.FALSE) + "_" + e, b = a.check_Focus
			...
			repairAllChk : function(c, a) {
				if (c.check.enable && c.check.chkStyle === j.checkbox.STYLE)
					for (var b = c.data.key.checked, d = c.data.key.children, g = f
							.getRoot(c), l = 0, h = g[d].length; l < h; l++) {
						var i = g[d][l];
						i.nocheck !== !0 && (i[b] = a);
			...
			repairSonChkDisabled : function(c, a, b) {
				if (a) {
					...
					if (a[d])
						for (var g = 0, l = a[d].length; g < l; g++)
			...
			setParentNodeCheckBox : function(c, a, b, d) {
				var g = c.data.key.children, l = c.data.key.checked, h = m("#"
						+ a.tId + j.id.CHECK);
				d || (d = a);
				f.makeChkFlag(c, a);
				a.nocheck !== !0
						&& a.chkDisabled !== !0
						&& (a[l] = b, e.setChkClass(c, h, a), c.check.autoCheckTrigger
								&& a != d
								&& a.nocheck !== !0
								&& c.treeObj.trigger(j.event.CHECK, [c.treeId,
												a]));
				if (a.parentTId) {
					h = !0;
					if (!b)
						for (var g = a.getParentNode()[g], i = 0, o = g.length; i < o; i++)
							if (g[i].nocheck !== !0 && g[i][l]
									|| g[i].nocheck === !0
									&& g[i].check_Child_State > 0) {
								h = !1;
								break
							}
			...
			setSonNodeCheckBox : function(c, a, b, d) {
				if (a) {
					...
					if (a != f.getRoot(c) && a.chkDisabled !== !0) {
						...
						if (a.nocheck !== !0) {
							if (a[l] = b, !i)
								a.check_Child_State = a[g] && a[g].length > 0
										? b ? 2 : 0
										: -1
						} else
							a.check_Child_State = -1;
		...
		data : {
			...
			getTreeCheckedNodes : function(c, a, b, d) {
				if (!a)
					return [];
				for (var g = c.data.key.children, e = c.data.key.checked, h = b
						&& c.check.chkStyle == j.radio.STYLE
						&& c.check.radioType == j.radio.TYPE_ALL, d = !d
						? []
						: d, i = 0, o = a.length; i < o; i++) {
					if (a[i].nocheck !== !0 && a[i][e] == b
							&& (d.push(a[i]), h))
						break;
			...
			getTreeChangeCheckedNodes : function(c, a, b) {
				if (!a)
					return [];
				for (var d = c.data.key.children, g = c.data.key.checked, b = !b
						? []
						: b, e = 0, h = a.length; e < h; e++)
					a[e].nocheck !== !0 && a[e][g] != a[e].checkedOld
							&& b.push(a[e]), f.getTreeChangeCheckedNodes(c,
							a[e][d], b);
				return b
			},
	...
	var n = m.fn.zTree, k = n._z.tools, j = n.consts, e = n._z.view, f = n._z.data;
	f.exSetting(v);
	...
	f.addInitProxy(function(c) {
		...
		if (k.eqs(c.type, "mouseover")) {
			if (b.check.enable && k.eqs(a.tagName, "span")
				...
				d = a.parentNode.id, l = "mouseoverCheck"
		} else if (k.eqs(c.type, "mouseout")) {
			if (b.check.enable && k.eqs(a.tagName, "span")
					&& a.getAttribute("treeNode" + j.id.CHECK) !== null)
				d = a.parentNode.id, l = "mouseoutCheck"
		} else if (k.eqs(c.type, "click") && b.check.enable
				&& k.eqs(a.tagName, "span")
				&& a.getAttribute("treeNode" + j.id.CHECK) !== null)
			d = a.parentNode.id, l = "checkNode";
		if (d.length > 0)
			switch (e = f.getNodeCache(b, d), l) {
				case "checkNode" :
					h = p;
					break;
				case "mouseoverCheck" :
	...
	f.addBeforeA(function(c, a, b) {
				...
				c.check.enable
						&& (f.makeChkFlag(c, a), c.check.chkStyle == j.radio.STYLE
								&& c.check.radioType == j.radio.TYPE_ALL
								&& a[d]
								&& f.getRoot(c).radioCheckedList.push(a), b
								.push("<span ID='", a.tId, j.id.CHECK,
	...
	f.addZTreeTools(function(c, a) {
		...
		a.getCheckedNodes = function(a) {
			...
			return f.getTreeCheckedNodes(this.setting, f.getRoot(c)[b],
					a !== !1)
		};
		...
		a.updateNode = function(c, f) {
			...
			if (c && this.setting.check.enable && m("#" + c.tId).get(0)
					&& k.uCanDo(this.setting)) {
...
(function(k) {
	var F = {
		...
		onHoverOutNode : function(b) {
			var b = p.getSetting(b.data.treeId), a = p.getRoot(b);
			if (a.curHoverNode && !p.isSelectedNode(b, a.curHoverNode))
				f.removeTreeDom(b, a.curHoverNode), a.curHoverNode = null
		},
		onMousedownNode : function(b, a) {
			function c(b) {
				if (z.dragFlag == 0
						&& Math.abs(K - b.clientX) < g.edit.drag.minMoveSize
						&& Math.abs(L - b.clientY) < g.edit.drag.minMoveSize)
					return !0;
				var a, c, e, j, l;
				...
				if (z.dragFlag == 0) {
					...
					j = -1;
					if (m.length > 1) {
						var s = m[0].parentTId ? m[0].getParentNode()[l] : p
								.getNodes(g);
						l = [];
					...
					e
							&& (D = m[0].getPreNode(), E = m[m.length - 1]
									.getNextNode());
					...
					for (a = 0, c = m.length; a < c; a++)
						if (e = m[a], e.editNameFlag = !1, f.selectNode(g, e,
								...
								.removeClass(d.node.CURSELECTED), y.append(j), a == g.edit.drag.maxShowNodeNum
								- 1) {
							j = k("<li id='" + e.tId
									+ "_moretmp'><a>  ...  </a></li>");
							y.append(j);
				...
				if (z.dragFlag == 1) {
					...
					c = b.clientY + B - l.top;
					var o = i.treeObj.height() + l.top - b.clientY - B, n = b.clientX
							+ j - l.left, H = i.treeObj.width() + l.left
							- b.clientX - j;
					l = c < g.edit.drag.borderMax && c > g.edit.drag.borderMin;
					var s = o < g.edit.drag.borderMax
							&& o > g.edit.drag.borderMin, I = n < g.edit.drag.borderMax
							&& n > g.edit.drag.borderMin, C = H < g.edit.drag.borderMax
							&& H > g.edit.drag.borderMin, o = c > g.edit.drag.borderMin
							&& o > g.edit.drag.borderMin
							&& n > g.edit.drag.borderMin
							&& H > g.edit.drag.borderMin, n = l
							&& i.treeObj.scrollTop() <= 0, H = s
							&& i.treeObj.scrollTop() + i.treeObj.height() + 10 >= a, M = I
							&& i.treeObj.scrollLeft() <= 0, N = C
					...
					if (o
						...
						if (!r && (b.target.id == i.treeId || n || H || M || N)
								&& (G || !G && e.parentTId))
							r = i.treeObj;
					...
					if (r && r.attr("id") != i.treeId) {
						...
						e = (c || !l)
								&& h.apply(i.edit.drag.prev, [i.treeId, m, w],
										!!i.edit.drag.prev);
						a = (c || !a)
								&& h.apply(i.edit.drag.next, [i.treeId, m, w],
										!!i.edit.drag.next);
						C = (c || !j)
								&& !(i.data.keep.leaf && !w.isParent)
								&& h.apply(i.edit.drag.inner, [i.treeId, m, w],
										!!i.edit.drag.inner);
						if (!e && !a && !C) {
							...
							if (w
								...
								window.zTreeMoveTimer = setTimeout(function() {
									v == d.move.TYPE_INNER
											...
											&& w.isParent
											&& !w.open
											&& (new Date).getTime() - J > i.edit.drag.autoOpenTime
											&& h.apply(
													i.callback.beforeDragOpen,
													[i.treeId, w], !0)
											&& (f.switchNode(i, w), i.edit.drag.autoExpandTrigger
													&& i.treeObj.trigger(
															d.event.EXPAND,
															[i.treeId, w]))
			...
			function q(b) {
				...
				if (z.dragFlag != 0) {
					...
					y && y.remove();
					t && t.remove();
					var o = b.ctrlKey && g.edit.drag.isMove
							&& g.edit.drag.isCopy || !g.edit.drag.isMove
							&& g.edit.drag.isCopy;
					!o && r && u && m[0].parentTId && u == m[0].parentTId
					...
					if (r) {
						...
						if (h.apply(g.callback.beforeDrop, [i.treeId, m, n, v,
							...
							a = function() {
								...
								} else if (o && v == d.move.TYPE_INNER)
									f.addNodes(i, n, s);
								else if (o
										&& f.addNodes(i, n.getParentNode(), s), v != d.move.TYPE_NEXT)
									for (b = 0, a = s.length; b < a; b++)
										f.moveNode(i, n, s[b], v, !1);
								else
									for (b = -1, a = s.length - 1; b < a; a--)
			...
			if (p.isSelectedNode(g, a))
				for (l = 0, j = n.length; l < j; l++) {
					if (n[l].editNameFlag && h.eqs(o.tagName, "input")
							&& o.getAttribute("treeNode" + d.id.INPUT) !== null)
						return !0;
					m.push(n[l]);
			...
			var x = k(document), y, t, r, G = !1, i = g, D, E, P = null, Q = null, u = null, v = d.move.TYPE_INNER, K = b.clientX, L = b.clientY, J = (new Date)
					.getTime();
	...
	k.extend(!0, k.fn.zTree._z, {
		tools : {
			...
			showIfameMask : function(b, a) {
				for (var c = p.getRoot(b); c.dragMaskList.length > 0;)
		...
		view : {
			...
			editNode : function(b, a) {
				var c = p.getRoot(b);
				f.editNodeBlur = !1;
				if (p.isSelectedNode(b, a) && c.curEditNode == a
						&& a.editNameFlag)
					setTimeout(function() {
			...
			moveNode : function(b, a, c, q, e, l) {
				...
				if (a != c
						&& (!b.data.keep.leaf || !a || a.isParent || q != d.move.TYPE_INNER)) {
					var h = c.parentTId ? c.getParentNode() : j, o = a === null
							|| a == j;
					o && a === null && (a = j);
					if (o)
						q = d.move.TYPE_INNER;
					j = a.parentTId ? a.getParentNode() : j;
					if (q != d.move.TYPE_PREV && q != d.move.TYPE_NEXT)
						q = d.move.TYPE_INNER;
					...
					var n;
					o ? n = o = b.treeObj : (!l && q == d.move.TYPE_INNER ? f
							.expandCollapseNode(b, a, !0, !1) : l
							|| f.expandCollapseNode(b, a.getParentNode(), !0,
									!1), o = k("#" + a.tId), n = k("#" + a.tId
							+ d.id.UL), o.get(0)
							&& !n.get(0)
							&& (n = [], f.makeUlHtml(b, a, n, ""), o.append(n
									.join(""))), n = k("#" + a.tId + d.id.UL));
					var m = k("#" + c.tId);
					m.get(0) ? o.get(0) || m.remove() : m = f.appendNodes(b,
							c.level, [c], null, !1, !0).join("");
					n.get(0) && q == d.move.TYPE_INNER ? n.append(m) : o.get(0)
							&& q == d.move.TYPE_PREV ? o.before(m) : o.get(0)
							&& q == d.move.TYPE_NEXT && o.after(m);
					var x = -1, y = 0, t = null, o = null, r = c.level;
					if (c.isFirstNode) {
						if (x = 0, h[g].length > 1)
							t = h[g][1], t.isFirstNode = !0
					} else if (c.isLastNode)
						x = h[g].length - 1, t = h[g][x - 1], t.isLastNode = !0;
					else
					...
					if (q == d.move.TYPE_INNER) {
						a[g] || (a[g] = []);
						if (a[g].length > 0)
							o = a[g][a[g].length - 1], o.isLastNode = !1;
						a[g].splice(a[g].length, 0, c);
						c.isLastNode = !0;
						c.isFirstNode = a[g].length == 1
					} else
						a.isFirstNode && q == d.move.TYPE_PREV
								? (j[g].splice(y, 0, c), o = a, o.isFirstNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !0, c.isLastNode = !1)
								: a.isLastNode && q == d.move.TYPE_NEXT
										? (j[g].splice(y + 1, 0, c), o = a, o.isLastNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !1, c.isLastNode = !0)
										: (q == d.move.TYPE_PREV ? j[g].splice(
												y, 0, c) : j[g].splice(y + 1,
					...
					!b.data.keep.parent && h[g].length < 1
							? (h.isParent = !1, h.open = !1, a = k("#" + h.tId
									+ d.id.UL), q = k("#" + h.tId + d.id.SWITCH), g = k("#"
									+ h.tId + d.id.ICON), f.replaceSwitchClass(
									h, q, d.folder.DOCU), f.replaceIcoClass(h,
				...
				if (c) {
					...
					if (c[d])
						for (var a = 0, e = c[d].length; a < e; a++)
							c[d][a] && p.setSonNodeLevel(b, c, c[d][a])
				}
			}
		}
	});
		...
		a.moveNode = function(a, q, e, l) {
			...
			if (a && !a.isParent && b.data.keep.leaf && e === d.move.TYPE_INNER)
				return null;
					...
					&& (q.parentTId == a.tId && e == d.move.TYPE_INNER || k("#"
							+ q.tId).find("#" + a.tId).length > 0))
				return null;
			else
				a || (a = null);
	...
	f.cancelPreSelectedNode = function(b, a) {
		for (var c = p.getRoot(b).curSelectedList, d = 0, e = c.length; d < e; d++)
			if (!a || a === c[d])
	...
	f.selectNode = function(b, a, c) {
		var d = p.getRoot(b);
		if (p.isSelectedNode(b, a) && d.curEditNode == a && a.editNameFlag)
			return !1;
