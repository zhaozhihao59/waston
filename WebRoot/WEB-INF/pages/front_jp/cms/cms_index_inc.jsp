	...
	<div class="fr actright-box">
		<div class="sctr-right-box">
			<div class="srb-nav-box fl">
				...
				<c:if test="${item.id=='439c9076-e890-11e4-ae9b-00266c0e7760'}">業界の動向</c:if>
			</div>
				...
				<div class="news-brief">
					<div class="news-image fl mr15">
						<c:if test="${empty article.path}">
							<a target="_blank" class="news-image-a" href="jp/cmsDetail_jp.htm?item.id=${article.id}"></a>
						</c:if>
						<c:if test="${!empty article.path}">
							<a target="_blank" class="news-image-a" href="jp/cmsDetail_jp.htm?item.id=${article.id}"><img src="${article.path}" width="160px" height="110px"/></a>
						</c:if>
					</div>
					<c:if test="${empty article.path}">
					<div class="news-content-image fl" style="width:885px;">
						...
						<div class="news-remark fl">
							<a c class="news-remark-a" href="jp/cmsDetail_jp.htm?item.id=${article.id }">
	...
	<!-- 页数一部提交 -->
 	<script type="text/javascript">
		$(".pageReslt").click(function (){
			...
			var id = $("#itemId").val();
			// 相关证书异步请求
			$.post(base+"jp/cms_ListPage_inc_jp.htm",{"page":currentPage,"item.id":id},function(data){
			 	$("#hcActive").html(data);
			});
		}) ;
