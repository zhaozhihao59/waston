	...
	<div class="act-index-middlebox">
		<div class="fl act-left-box">
			...
			<ul class="act-left-ul">
				<li class="cur"><a href="front/activity_jp/activities_ListPage_jp.htm" class="jzhd-ico">イベント</a></li>
			</ul>
			<div class="mt10">
		...
		<div class="fr actright-box">
			...
			<div class="hd-list-box">
				...
				<div class="fr hd-ccbox">
					<div class="hd-cc-tt clb"> ${item.titleJp } <a href="front/activity_jp/activities_ListPage_jp.htm" class="goback-ico fr" style="margin-top:0;font-size:14px;">戻る</a></div>
					<div class="addr-date mt5">
						<label>開催日：</label><span>${item.dateStr }</span>
						<label class="ml30">開催場所：</label><span>${item.addressJp }</span>
					</div>
				<%-- 	<div class="scl-act-content mt5">
						${dv:e(item.prom,180)}
					</div> --%>
					<s:if test="item.state == 2">
						<div class="cp ljbm-btn yjfk"  data="${item.id }">申込み</div>
					</s:if>
			...
			<div class="pd20">
				...
				<div class="bjq-css">${item.descJp }</div>
			</div>	
		</div>
		<div class="cb"></div>
	</div>
