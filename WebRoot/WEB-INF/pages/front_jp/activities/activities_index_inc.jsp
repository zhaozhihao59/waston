		...
		<c:forEach var="activity" items="${pageResult.result}">	 
			<li>
				<div class="sclhdyg-imgbox">
					<a target="_blank" href="front/activity_jp/activitiesDetail_jp.htm?item.id=${activity.id }">
						<img src="${activity.photo }">
					</a>
				</div>
				<div class="sclhdyg-name">
					<a target="_blank" href="front/activity_jp/activitiesDetail_jp.htm?item.id=${activity.id }" title="${activity.titleJp}">${dv:e(activity.titleJp,40)}</a>
				</div>
				<div class="sclhdyg-time-box">
					開催日：${activity.dateStr}
				</div>
	...
	<!-- 页数异步提交 -->
 	<script type="text/javascript">
		$(".pageReslt").click(function (){
			...
			// 相关证书异步请求
			$.post(base+"front/activity_jp/ListPageByOld_jp.htm",{"page":currentPage},function(data){
			 	$("#pageinc").html(data);
			});
		}) ;
