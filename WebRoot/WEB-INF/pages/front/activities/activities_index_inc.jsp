		...
		<c:forEach var="activity" items="${pageResult.result}">	 
			<li>
				<div class="sclhdyg-imgbox">
					<a target="_blank" href="front/activity/activitiesDetail.htm?item.id=${activity.id }">
						<img src="${activity.photo }">
					</a>
				</div>
				<div class="sclhdyg-name">
					<a target="_blank" href="front/activity/activitiesDetail.htm?item.id=${activity.id }" title="${activity.title}">${dv:e(activity.title,40)}</a>
				</div>
	...
	<!-- 页数异步提交 -->
 	<script type="text/javascript">
		$(".pageReslt").click(function (){
			...
			// 相关证书异步请求
			$.post(base+"front/activity/ListPageByOld.htm",{"page":currentPage},function(data){
			 	$("#pageinc").html(data);
			});
		}) ;
